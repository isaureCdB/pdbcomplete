...
"--patch X None" with X the residue index of the 1st nucl; this will keep the
...
def read_models(pdb):
    ...
    for l in open(pdb):
        ...
        if ll[0] == "MODEL":
            ...
            outp_name = "/tmp/model%i.pdb"%ll[1]
            outp = open(outp_name)
            pdbs.append([])
...
parser.add_argument("--patch", help="Provide a residue number and a patch name to apply",
                    dest="patches", nargs=2, action="append",default=[])
