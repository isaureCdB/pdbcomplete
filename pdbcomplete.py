...
def write_pdb(pdbres, args, patches={}, heavy = False, one_letter_na = False, write_missing=True, pdb2pqr=False):
    pdblines = []
    mapping = []
    mutations_5ter = {}
    firstchain = True
    ...
    for res in pdbres:
        ...
        for a in top.atomorder:
            ...
            pdblines.append("ATOM%7d %4s%4s %s%4s   %s%7.3f 1.00" % \
              (atomcounter, atomname, resname, res.chain, res.resid, xyz, atom["charge"]))
            atomcounter += 1
